...
\subsection{The Transparency Paradox in Multi-Agent CDSS}
Clinical decision support systems (CDSS) are fundamental to modern diagnostic accuracy and patient safety \cite{sutton2020cdss,miyachi2023learning}. However, their integration is frequently hampered by ``alert fatigue'' and poor workflow alignment, where excessive or opaque information overwhelms clinician cognitive capacity \cite{marcilly2018alerts,bayor2025cdss,harrison2022patient}. As CDSS architectures evolve from monolithic models to multi-agent large language model (LLM) systems, this usability challenge has intensified \cite{chenx2025diagnostic,chen2025enhancing}.

Multi-agent frameworks, such as ArgMed, enhance diagnostic reasoning by distributing tasks across specialized roles \cite{chen2025enhancing,hong2024argmed,peng2025tree}. While this decomposition improves robustness, it generates high-velocity, interleaved reasoning traces that are inherently difficult for humans to parse in real time \cite{hong2024argmed}. The resulting \textit{transparency paradox} is that the system becomes more accurate but less intelligible. Traditional post-hoc Explainable AI (XAI) methods, such as feature importance or saliency maps, are ill-suited for this domain \cite{abbas2025xai,salimiparsa2021design}. They explain model sensitivity rather than the evolving clinical narrative and often lack the temporal immediacy required for rapid decision-making \cite{vilone2021explainable,benzion2025anxiety}. Consequently, clinicians increasingly prioritize concise, natural language explanations over probabilistic metrics \cite{silva2023xai,schoonderwoerd2021patterns}, necessitating a shift from model-centric to process-centric transparency \cite{derksen2025cdss}.

\subsection{Real-Time Summarization and Protocol Alignment}
To resolve the conflict between agent verbosity and clinical time constraints, recent research focuses on automated summarization rather than raw exposure. However, most dialogue systems rely on ``turn-based'' interaction paradigms, which artificially delay updates until an agent completes a generation \cite{leduc2025speech,wu2025incremental}. In high-throughput multi-agent scenarios, this latency is unacceptable; information must be processed as a stream. Studies in meeting summarization and customer support demonstrate that \textit{incremental} updates triggered by semantic events---rather than fixed time intervals or turn boundaries---significantly improve information retention and reduce redundancy \cite{schneider2024meeting,wu2025incremental,leduc2025speech}.

Furthermore, clinical utility depends on strict adherence to communication standards. The SBAR (Situation-Background-Assessment-Recommendation) \cite{haig2006sbar} and SOAP (Subjective-Objective-Assessment-Plan) \cite{podder2023soap} frameworks are the gold standards for concise medical communication, designed to minimize error during handoffs \cite{krishna2021soap,zhang2024annotate}. While LLMs have demonstrated proficiency in generating these formats from completed documents \cite{vanveen2023summarization,fraile2025llm}, there is a distinct gap in middleware capable of enforcing these schemas dynamically over live, interleaved reasoning streams. EXAIM addresses this by fusing streaming semantic detection with schema-constrained synthesis, ensuring that real-time outputs remain strictly aligned with established clinical mental models \cite{yim2023acibench,bailly2025divide}.
