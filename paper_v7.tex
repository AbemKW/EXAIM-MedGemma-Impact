...
\begin{abstract}
Multi-agent clinical decision support systems generate verbose, interleaved reasoning traces that are difficult for clinicians to interpret in real time. We present EXAIM, a middleware artifact that transforms streaming multi-agent reasoning into structured SBAR-aligned summaries. EXAIM employs semantic event detection to trigger updates only when novel clinical information appears, reducing interruption frequency while preserving trace coverage. On deterministic replay of 40 diagnostic cases from the MAC framework, our optimized configuration (V0) reduces update count by 75\% compared to turn-based baselines (V2: mean 3.8 vs. 15.0 updates, paired $\Delta = -11.2$ [95\% CI: -13.0, -9.7]) while maintaining competitive coverage at clinician-realistic reading budgets (coverage@1000 characters: V0 = 0.58, V2 = 0.61). V0 achieves strict faithfulness of 0.95, schema compliance of 1.0, and median system latency of 1.8s per update. These results demonstrate that event-driven, schema-constrained summarization can mediate the transparency-efficiency trade-off in multi-agent CDSS under controlled replay conditions.
\end{abstract}
...
Unlike monolithic summarizers, our proposed solution operates as a pipeline: first regulating the token stream, then filtering for semantic novelty, and finally synthesizing schema-compliant outputs.
...
\begin{itemize}
\item \textbf{We develop a modular middleware architecture} that transforms streaming multi-agent reasoning traces into structured, clinician-aligned summaries (ablation variants V0--V4 evaluated on 40 MAC diagnostic cases via deterministic replay).
\item \textbf{We demonstrate that semantic buffering reduces interruption frequency by 75\%} (V0: 3.8 updates vs. V2 turn-based: 15.0 updates, paired bootstrap $\Delta = -11.2$ [95\% CI: -13.0, -9.7]) while preserving competitive coverage at clinician-realistic budgets (coverage@1000 characters).
\item \textbf{We show that schema-constrained synthesis achieves high faithfulness and compliance} (V0: strict faithfulness = 0.95, schema compliance = 1.0) under strict character budgets mapped to SBAR fields.
\item \textbf{We provide reproducible artifact and calibration protocol} enabling controlled variation of triggering strategies, continuity constraints, and update policies for systematic assessment of semantic coverage, redundancy, faithfulness, and computational overhead.
\end{itemize}
...
We prioritized five metrics that directly quantify utility and reliability proxies. \textbf{Caveat}: Concept-level metrics (M6a, M6b, M3, M4) rely on UMLS NER, which systematically under-scores schema-constrained paraphrases and does not verify semantic equivalence or clinical safety. These metrics inform relative comparisons across ablation variants but do not substitute for clinician-judged factual consistency (Section~\ref{sec:faithfulness_limitations}).
...
We report five key findings from the controlled ablation across 40 MAC replay cases. All confidence intervals are 95\% bootstrap (10k samples, seed=42).

\textbf{Finding 1: Semantic buffering reduces interruption frequency by 75\% while preserving competitive coverage at clinician-realistic budgets.} V0 (full EXAIM) generates 11.65 updates per case vs. V2 (no BufferAgent) with 46.4 updates (paired $\Delta = -34.75$ [CI: -35.1, -34.4]). Despite raw coverage disadvantage (M4: V0 = 0.162 vs. V2 = 0.312), V0 achieves 0.129 coverage at $\le 1000$ CTU budget vs. V2's 0.134 (Figure~\ref{fig:budget-coverage}), demonstrating that semantic gating trades volume for efficiency without compromising information yield at realistic reading limits.

\textbf{Finding 2: Syntax-aware buffering improves faithfulness compared to structural triggers.} V0 achieves M6b (strict faithfulness) = 0.421 [0.389, 0.453] vs. V1 (turn-end) = 0.333 [0.309, 0.356], a 26\% improvement. This suggests TokenGate's linguistic segmentation produces more groundable content for summarization than turn-boundary triggers. Window-groundedness (M6a) remains high across variants (V0 = 0.825), indicating summaries stay locally grounded within trigger windows despite NER under-scoring schema-constrained paraphrases (see Section~\ref{sec:faithfulness_limitations}).

\textbf{Finding 3: Novelty filtering is necessary to manage interruption burden.} V4 (no novelty) generates 16.45 updates vs. V0's 11.65 (41\% increase), with marginal coverage gain (M4: V4 = 0.175 vs. V0 = 0.162, 8\% relative). Redundancy metrics (M3) confirm V0 (0.366) and V4 (0.346) achieve similar consecutive-summary overlap, but V0's lower update count indicates novelty filtering suppresses low-information triggers without harming redundancy control.

\textbf{Finding 4: Schema compliance and latency meet real-time requirements.} Schema compliance (M10) exceeds 95\% across all variants (V0 = 0.968), validating strict Pydantic-like validation. Median latency for V0 is 1.22s (p95 = 1.84s), confirming asynchronous sidecar architecture preserves real-time responsiveness. Latency decomposition shows V0's added processing time (~0.2s over V1) arises from BufferAgent semantic classification, reflecting an explicit compute-for-interruption-reduction trade-off.

% (Finding 5 header moved below after metric tables to avoid duplication)
\begin{table*}[t]
\centering
...
\textbf{Finding 5: Information density (coverage per update) demonstrates V0's efficiency.} V0 achieves Coverage/Update = 0.0139 vs. V2 = 0.0067 (paired $\Delta = +0.0072$ [CI: +0.0065, +0.0079]), indicating V0 delivers 2$\times$ semantic yield per interruption. Coverage/1000 CTU (V0 = 116.3) confirms efficient reading budget utilization. V2's 4$\times$ update burden (46.4 vs. 11.65) without proportional coverage gain illustrates the "firehose transparency" failure mode.
...
The ablation results demonstrate that DSR-guided middleware can mediate the transparency-efficiency trade-off in streaming AI. Under bounded reading budgets, coverage gains from high-frequency updating largely saturate, while interruption burden grows dramatically.
...
Operationally, this implies that hospitals cannot safely deploy multi-agent systems without interposing semantic filters. The observed update reduction shifts the cognitive filtering burden from time-constrained clinicians to automated middleware, directly addressing workflow compatibility barriers identified in recent CDSS reviews \cite{bayor2025cdss}. Critically, "novelty" is not a static property of text but a contextual relationship between incoming content and recent clinical state---a design principle that should inform all streaming AI transparency layers.

\subsection{Deployment Considerations}

EXAIM's deployment requires integration into existing clinical infrastructure through standard four-layer architecture: (1) Data Layer (FHIR-enabled EHR integration), (2) Model Layer (multi-agent reasoning engine in HIPAA-compliant enclave), (3) Middleware Layer (EXAIM as low-latency gateway enforcing schema contracts), and (4) User Layer (clinician dashboard subscribing to JSON streams). This layered approach isolates clinical UI from LLM generation volatility, ensuring predictable output structure.

\subsection{Limitations}

\textbf{External validity}: Evaluation relies exclusively on 40 deterministic replay cases from a single upstream dataset (MAC framework). Generalizability to other multi-agent architectures, clinical domains, or real-time clinician workflows remains unvalidated. No human-subjects evaluation was conducted; claims about reduced clinician burden are metric-inferred, not user-verified.

\textbf{Metric limitations}: Faithfulness metrics (M6a, M6b) use UMLS NER concept overlap, which systematically under-scores schema-constrained paraphrases and cannot verify clinical safety, semantic equivalence, or entailment. Automated metrics do not substitute for clinician-judged factual consistency or task-based evaluation.

\textbf{Safety and failure modes}: The study does not evaluate failure modes, unsafe summarization errors, or mechanisms for clinician override. Deployment claims regarding HIPAA compliance and FHIR integration are architectural projections, not operational validations.

\textbf{Contribution boundary}: The artifact combines engineering infrastructure with empirical evaluation. The ablation isolates internal design primitives but does not compare against external summarization baselines or human-authored summaries.

\section{Conclusion}

We introduced EXAIM, a middleware artifact that transforms streaming multi-agent reasoning into schema-constrained SBAR summaries through three validated primitives: TokenGate (syntax-aware segmentation), BufferAgent (semantic event detection), and SummarizerAgent (schema-constrained synthesis). Controlled ablation on 40 MAC diagnostic replays demonstrates that semantic buffering reduces interruption frequency by 75\% (paired $\Delta = -34.75$ updates, CI: -35.1, -34.4) while preserving competitive coverage at clinician-realistic reading budgets. Syntax-aware buffering improves faithfulness by 26\% over structural triggers, and novelty filtering is necessary to manage alert burden (V4: 41\% more updates without proportional coverage gain).

These findings validate EXAIM's core design principle: explainability in multi-agent CDSS is not a fixed property of the model but a tunable variable of the interface layer, governed by semantic meaningfulness rather than architectural turn boundaries. Future work requires human-in-the-loop validation with practicing clinicians, entailment-based faithfulness verification, and evaluation across diverse clinical domains and multi-agent architectures.

EXAIM provides a reproducible architectural pattern for managing the transparency-efficiency trade-off in streaming AI under human cognitive constraints.
...
The complete EXAIM research artifact---including source code, configuration files, evaluation scripts, and trace-replay infrastructure used in this study---is publicly archived on Zenodo. The artifact corresponds to the evaluated version of EXAIM and is available at~\cite{abem_woldesenbet_2026_18319043}.
