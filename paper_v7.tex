...
\begin{abstract}
Multi-agent Large Language Models (LLMs) offer diverse diagnostic perspectives but generate verbose, interleaved reasoning traces that overwhelm clinicians. We present EXAIM, a middleware artifact that transforms streaming multi-agent reasoning into structured summaries aligned with standard clinical communication protocols (SBAR/SOAP). Unlike simple turn-based summarizers, EXAIM employs semantic event detection to trigger updates only when novel clinical information appears.

In a controlled ablation study on rare-disease diagnostic traces, EXAIM reduces update frequency by 75\% compared to non-filtering baselines (11.7 vs. 46.4 updates per case) while maintaining competitive information coverage under strict, clinically aligned length constraints. The system achieves strict schema compliance (96.8\%) and a median latency of 1.22s, validating its feasibility for real-time deployment. These results demonstrate that event-driven, schema-constrained summarization can effectively mediate the tension between diagnostic transparency and clinical workflow efficiency.
\end{abstract}
...
We develop EXAIM as a solution-oriented IT artifact designed to optimize information density and reduce alert frequency in multi-agent CDSS \cite{hevner2004design,hor2025design}. This section describes the artifact's architecture, key components, and operational logic. We first present the overall framework, then detail the three primary modules (TokenGate, BufferAgent, SummarizerAgent), and demonstrate system operation through a case walkthrough.
...
We verify the effectiveness of the EXAIM artifact through a controlled ablation study using deterministic replays of multi-agent reasoning traces. This approach allows us to stress-test the middleware's flow-control capabilities independent of variations in upstream agent behavior.
...
We prioritized six metrics that directly quantify utility and reliability proxies:

\begin{itemize}
    \item \textbf{Window-Groundedness (M7):} A window-grounded variant of faithfulness that measures the fraction of summary concepts grounded within a local sliding window of trace content centered on the summary's trigger:
    \begin{equation}
    ...
    \end{equation}
    This metric is \emph{not normalized by update count}, and therefore inherently favors high-frequency systems. M5 (Budget-Constrained Coverage) provides the normalized coverage view.
...
Notably, faithfulness is a standard evaluation criterion in human-in-the-loop dialogue systems \cite{chen2022hitl}; however, concept-level proxies such as M6a and M6b measure explicit entity overlap and therefore can understate semantic fidelity when paraphrase, normalization, or schema-driven compression occurs.

Despite these limitations, concept-level metrics remain informative for relative comparisons across ablation variants under identical extraction assumptions. Future work should incorporate clinician-judged factual consistency, entailment verification, and task-based evaluation to complement automated assessment.
...
Information density further illustrates V0's efficiency: V0 achieves Coverage/Update = 0.0139 vs. V2 = 0.0067 (paired $\Delta = +0.0072$ [CI: +0.0065, +0.0079]), indicating V0 delivers roughly 2$\times$ semantic yield per clinician-facing interruption. V2's 4$\times$ update burden (46.4 vs. 11.65) without proportional coverage gain exemplifies the "firehose transparency" failure mode.

\textbf{Finding 2 (RQ1c, RQ1d): Syntax-aware buffering improves faithfulness compared to structural triggers.} V0 achieves M6 (strict faithfulness) = 0.421 [0.389, 0.453] vs. V1 (turn-end) = 0.333 [0.309, 0.356], a 26\% improvement. This suggests TokenGate's linguistic segmentation produces more groundable content for summarization than turn-boundary triggers. Window-groundedness (M7) remains high across variants (V0 = 0.825), indicating summaries stay locally grounded within trigger windows despite NER under-scoring schema-constrained paraphrases (see Section~\ref{sec:faithfulness_limitations}).
...
Faithfulness \textit{(M6)} & 0.421 & 0.333 & 0.409 & 0.382 & 0.424 \\
...
We introduced EXAIM, a middleware artifact that transforms streaming multi-agent reasoning into schema-constrained summaries through three validated primitives: TokenGate (syntax-aware segmentation), BufferAgent (semantic event detection), and SummarizerAgent (schema-constrained synthesis). Controlled ablation on 40 MAC diagnostic replays demonstrates that semantic buffering substantially reduces interruption frequency while preserving competitive coverage at clinician-realistic reading budgets, and that syntax-aware buffering improves faithfulness over structural triggers.

These findings validate EXAIM's core design principle: controlling when to explain is as vital as how to explain in multi-agent CDSS. Future work requires human-in-the-loop validation with practicing clinicians, entailment-based faithfulness verification, and evaluation across diverse clinical domains and multi-agent architectures.
