...
class OrchestratorAgent(BaseAgent):
    ...
    async def act_stream(self, input: str, token_callback: Optional[Callable[[str], None]] = None) -> AsyncIterator[str]:
        """Stream tokens as they are generated by the LLM
        
        Args:
            input: Input text for the agent
            token_callback: Optional callback function to call immediately when each token is yielded
        ...
        try:
            async for chunk in chain.astream({"input": input}):
                ...
                if hasattr(chunk, 'content'):
                    ...
                    if content:
                        # Call callback immediately if provided (before yielding)
                        if token_callback:
