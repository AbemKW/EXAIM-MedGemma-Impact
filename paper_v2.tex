...
Therefore, We propose EXAIM (Explainable AI Middleware), a real-time summarization architecture designed to bridge the gap between multi-agent reasoning complexity and clinical information needs. The following research questions are presented:
...
The middleware maintains internal timestamps for timeout detection but does not require external timing metadata from agents. For evaluation purposes, frozen trace replays include additional metadata (absolute timestamps, turn IDs, sequence numbers) to enable deterministic replay, but these fields are not part of the runtime middleware contract.

The middleware does not require modification of upstream agent prompts, architectures, or orchestration logic. It operates in ``push'' mode, receiving text increments as they are generated without blocking or altering agent behavior.
...
EXAIM implements a three-stage pipeline that progressively transforms raw token streams into structured summaries. Figure~\ref{fig:system-architecture} illustrates the complete data flow. The pipeline pseudocode is presented below.
...
To illustrate EXAIM's semantic filtering and redundancy suppression in operation, we present a specific interaction sequence from a frozen multi-agent trace (Case ID: 3949). This example demonstrates how the middleware distinguishes between verbose repetition and genuinely novel clinical information.
...
V0 improves strict faithfulness by 26\% (0.421 vs. 0.333) and reduces redundancy by 20\% (0.366 vs. 0.456) relative to V1, with modest coverage gains (0.162 vs. 0.144) at a moderate latency cost (1.28s vs. 1.03s mean). As discussed in Section V-C, absolute faithfulness scores are systematically depressed by summarization effects; however, relative comparisons across variants remain informative under identical extraction assumptions.

While V0 generates 37\% more updates than V1 (11.65 vs. 8.5 updates per case), these updates contain higher informational value and less redundancy. This illustrates that semantic event detection does not reduce update frequency relative to turn-based triggering, but rather reallocates updates to semantically meaningful boundaries. Turn-based summarization compresses multi-topic agent turns into single summaries, leading to information loss and reduced grounding. The semantic event detection mechanism (RQ1d) allows EXAIM to surface clinically meaningful updates at sub-turn granularity, improving both faithfulness and coverage. The true benefit of V0's update frequency control becomes evident in comparison to V2 (no BufferAgent), where V0 achieves a 4$\times$ reduction in update frequency (11.65 vs. 46.4 updates per case) while maintaining superior faithfulness and substantially lower redundancy.

\subsubsection{Impact of Semantic Buffering (V0 vs. V2)}

Removing the semantic buffering layer (V2) results in a dramatic increase in update frequency (46.4 vs. 11.65 updates per case, 4.0$\times$ increase) and output volume (4313 vs. 1391 CTU, 3.1$\times$ increase). Without semantic filtering, the system produces excessive, fragmented updates directly from every TokenGate flush. While V2 achieves the highest coverage (0.312), this comes at the cost of extreme interruption frequency that would be operationally unusable in clinical settings.

This result confirms that syntax-aware segmentation alone is insufficient for real-time explainability (RQ1b). The BufferAgent's semantic filtering is essential for converting high-frequency, low-signal flushes into sparse, high-value clinician-facing updates. V2's slightly better redundancy score (0.348 vs. 0.366) is an artifact of measurement: with 4$\times$ more updates, individual summaries cover smaller semantic units, reducing overlap by virtue of fragmentation rather than improved filtering.

\subsubsection{Adaptive vs. Fixed Segmentation (V0 vs. V3)}

Replacing syntax-aware segmentation with fixed-size chunking (V3) degrades both coverage (0.134 vs. 0.162, 17\% reduction) and faithfulness (0.382 vs. 0.421, 9\% reduction). Fixed segmentation frequently splits semantically complete reasoning units mid-sentence or mid-thought, forcing the summarizer to operate on incomplete context. This fragmentation leads to:
...
\item \textbf{Higher Redundancy}: V3's redundancy (0.413) is worse than V0 (0.366), suggesting that fixed chunking creates artificial update boundaries that split coherent reasoning and force the summarizer to repeat context across fragments
\end{itemize}

Despite operating at comparable update frequencies (9.5 vs. 11.65 updates per case) and having access to the same BufferAgent filtering, V3's fixed boundaries systematically underperform adaptive syntax-aware segmentation. This demonstrates that TokenGate's boundary detection is not merely a latency optimization—it directly improves semantic coherence of the buffered units, enabling better downstream summarization (RQ1d).

Interestingly, V3 shows the highest LLM usage (81,694 CTU vs. 62,178 for V0), suggesting that BufferAgent must work harder to analyze fragmented, context-poor chunks, leading to longer prompts or more complex reasoning.

\subsubsection{Effect of Novelty Filtering (V0 vs. V4)}

Disabling novelty filtering (V4) increases update frequency by 41\% (16.45 vs. 11.65 updates per case) and output volume by 27\% (1763 vs. 1391 CTU) without materially improving redundancy (0.346 vs. 0.366). Although V4 captures marginally more information (coverage 0.175 vs. 0.162, 8\% gain), it does so at the cost of significantly higher interruption frequency.

V4 actually achieves the best faithfulness (0.424 vs. 0.421), but this marginal improvement (0.7\%) comes at the expense of 41\% more updates. This demonstrates that novelty filtering functions primarily as an update frequency control mechanism rather than a simple deduplication strategy (RQ1b). The novelty gate suppresses incremental updates that are technically novel but represent low marginal information gain, trading small coverage losses for substantial reductions in clinician interruptions.

The nearly identical redundancy between V0 and V4 (0.366 vs. 0.346) indicates that novelty filtering operates at a different level than consecutive-update similarity: it prevents semantically distinct but marginally informative updates rather than blocking redundant repetitions.

\subsubsection{Computational Overhead and Real-Time Viability}

EXAIM introduces additional computational cost due to semantic analysis of the reasoning stream. Average summary latency increases from 1.03s (V1) to 1.28s (V0), with p95 tail latencies of 1.59s (V1) and 1.84s (V0). In streaming systems, tail latencies are critical for user experience; the observed p95 values remain within acceptable bounds for interactive dashboard update latencies ($<$ 2 seconds).

Total LLM token-equivalent usage (M9) increases by 5.3$\times$ (62,178 vs. 11,618 CTU per case) from V1 to V0, reflecting the cost of semantic buffering and novelty analysis. While this represents significant computational overhead, it must be evaluated relative to the benefits: 26\% improved faithfulness, 20\% reduced redundancy, and event-driven triggering that aligns updates with clinical meaning rather than arbitrary turn boundaries.

The relationship between update frequency and LLM usage is not strictly linear. V2 (no BufferAgent) generates 4.0$\times$ more updates than V0 but incurs 40\% lower LLM usage (37,151 vs. 62,178 CTU), demonstrating that BufferAgent's semantic analysis adds per-decision overhead. However, V2's approach is clinically unusable due to extreme interruption frequency. V0 trades higher per-update analysis cost for substantially better filtering, achieving 75\% fewer updates than V2 (11.65 vs. 46.4) with superior faithfulness and redundancy suppression.

\subsubsection{Summary of Marginal Contributions}

\begin{itemize}
\item \textbf{TokenGate (syntax-aware)}: Provides coherent semantic units for downstream analysis; V3 shows that fixed chunking degrades coverage and faithfulness even with semantic filtering
\item \textbf{BufferAgent (semantic filtering)}: Essential for controlling update frequency; V2 shows 4.0$\times$ update inflation without it
\item \textbf{Novelty Gating}: Functions as update frequency regulator, suppressing low-marginal-value updates; V4 shows 41\% increase in interruptions for only 8\% coverage gain
\item \textbf{Event-Driven Triggering}: V1 comparison demonstrates value of semantic triggers vs. turn boundaries, improving faithfulness (26\%) and reducing redundancy (20\%)
\end{itemize}

These ablation results validate the design hypothesis that semantic-driven, multi-stage filtering is necessary for effective real-time explainability in multi-agent systems. Each component contributes to the system's ability to balance transparency (coverage, faithfulness) with usability (update frequency, redundancy suppression).
