...
Design Science aims to create and evaluate IT artifacts intended to solve identified organizational problems \cite{hevner2004design}. We adhere to the six design steps of DSR and the nominal process model outlined by Peffers et al. \cite{peffers2007dsr} in designing EXAIM. This approach aligns with recent applications of DSR for developing AI-based clinical tools that balance technical innovation with practical usability requirements \cite{hor2025design}. This section systematically provides a comprehensive description of the artifact design, EXAIM.

\subsection{System Model and Interface Assumptions}

EXAIM operates as a stateful observer middleware positioned between upstream multi-agent diagnostic systems and clinician-facing interfaces. The system processes asynchronous streams of text deltas and emits structured summaries at semantically detected events rather than fixed intervals or turn boundaries.

\subsubsection{Input Model}

The system assumes a stream-based input model where multiple upstream agents emit incremental text deltas asynchronously. Each delta is a tuple $(a, \tau)$ where $a$ is the agent identifier and $\tau$ is a text fragment (substring of ongoing generation). The middleware subscribes to this stream via callback registration, operating in push mode without blocking upstream generation. Timing information is inferred from arrival timestamps rather than explicit metadata, allowing integration with any agent framework supporting token-level streaming.
...
\item \textbf{V2 (Turn Baseline)}: Bypasses semantic evaluation entirely, triggering on agent turn boundaries
\item \textbf{V1 (No BufferAgent)}: Removes semantic layer, processing all TokenGate chunks
\end{itemize}
...
All evaluation code, configuration files, frozen traces, and metric computation scripts are provided in the /evals directory. Docker-based execution ensures environment reproducibility. The calibration process (Section IV-A) and metric definitions (Section IV-C) are fully documented and deterministic given fixed random seeds.
...
\item \textbf{Constraint Enforcement:} Schema validation uses Pydantic v2 with LangChain's structured output (strict=True) as described in Section III-D.3. For evaluation purposes, failed summaries are marked (schema\_ok=false) and excluded from metrics while the system continues operation.
