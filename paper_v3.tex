...
Generation proceeds through a three-stage pipeline. First, \textbf{Draft}: the LLM generates a structured prediction using strict Pydantic type definitions mapped to SBAR/SOAP fields (Table~\ref{tab:schema}). Second, \textbf{Validate}: the system checks whether the generated content respects all character limits. Third, \textbf{Repair}: if validation fails, the system iteratively invokes the LLM with increasingly aggressive compression instructions until the output fits or a maximum retry count is reached.
...
We utilized the Multi-Agent Conversation (MAC) framework \cite{chenx2025diagnostic} as the upstream trace generator. MAC is a multi-agent diagnostic system where diverse doctor agents and a supervisor collaborate to solve complex medical cases.
We instrumented MAC to capture real-time token dynamics, logging granular stream deltas and precise timestamps. These logs serve as a frozen replay dataset of 40 rare-disease diagnostic cases (seed=42), enabling deterministic simulation of live streams.
...
\begin{enumerate}
    ...
    \item \textbf{Baseline Calibration}: The fixed-chunk baseline (V3) was calibrated to the median flush size of the full system to ensure valid comparison.
