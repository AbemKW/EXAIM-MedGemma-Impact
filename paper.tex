...
This study follows the Design Science Research Methodology (DSRM) framework proposed by Peffers et al. \cite{b25}. Design science research is appropriate for this work as it focuses on creating and evaluating artifacts (in this case, the EXAIM middleware) to address identified problems in clinical decision support systems. The DSRM framework provides a structured approach to conducting design science research through six sequential steps, which we map to our study as follows:

\textbf{Step 1: Problem Identification} The problem addressed in this research stems from the limitations identified in Section I. Multi-agent clinical decision support systems generate verbose, interleaved reasoning traces that are difficult for clinicians to interpret during time-sensitive decision-making. Existing explainability approaches are poorly aligned with real-time clinical workflows, exposing internal model mechanics rather than supporting clinical sensemaking. These limitations highlight the need for a real-time, process-level explainability mechanism that operates during reasoning rather than after it.

\textbf{Step 2: Objectives} The objectives of this research are defined in Section IV (Artifact Design), where we establish the design goals for EXAIM. The primary objective is to create a middleware architecture that transforms streaming multi-agent reasoning traces into structured, clinician-aligned summaries without modifying upstream diagnostic agents. Secondary objectives include enabling semantic event-driven summarization, operationalizing process-level transparency through schema-constrained summaries, and empirically evaluating streaming explainability trade-offs.

\textbf{Step 3: Design \& Development} The design and development of the EXAIM artifact is detailed in Section IV, Subsection B (EXAIM Architecture). The architecture consists of three sequential components: (1) TokenGate for syntax-aware stream regulation, (2) BufferAgent for semantic event detection, and (3) SummarizerAgent for schema-constrained synthesis. Each component is designed to progressively increase semantic abstraction while enforcing flow-control policies.

\textbf{Step 4: Demonstration} The demonstration of the EXAIM artifact is provided through a case walkthrough in Section IV, Subsection C. This walkthrough illustrates how raw multi-agent reasoning traces are transformed into structured clinical summaries, demonstrating the artifact's utility in addressing the identified problem.

\textbf{Step 5: Evaluation} The evaluation of the EXAIM artifact is conducted through a controlled ablation study detailed in Section V (Evaluation). This evaluation compares EXAIM against turn-based and heuristic baselines using multiple metrics including coverage, redundancy, faithfulness, and computational overhead. The evaluation demonstrates that semantic buffering significantly improves trace coverage and faithfulness while reducing redundant updates.

\textbf{Step 6: Communication} This paper serves as the communication of the research, presenting the problem identification, artifact design, demonstration, and evaluation to the research community. The communication includes detailed descriptions of the architecture, design rationale, experimental methodology, and results.
...
[PLACEHOLDER: A specific example showing a raw input string from multi-agent reasoning traces being transformed into an EXAIM summary. This should demonstrate the complete pipeline from raw tokens through TokenGate, BufferAgent, and SummarizerAgent to the final structured summary output.]
...
The TokenGate component requires four configurable parameters: minimum word threshold (min\_words), maximum word threshold (max\_words), silence detection timer (silence\_timer), and maximum wait timeout (max\_wait\_timeout). To ensure reproducible parameter selection, we conducted systematic calibration across a grid of 625 policy combinations evaluated on frozen replay traces. The parameter grid explored min\_words $\in$ [30, 40, 50, 60, 70], max\_words $\in$ [80, 100, 120, 140, 160], silence\_timer $\in$ [1.0, 1.5, 2.0, 2.5, 3.0] seconds, and max\_wait\_timeout $\in$ [4.0, 5.0, 6.0, 7.0, 8.0] seconds, with validity constraints ensuring min\_words $<$ max\_words and max\_wait\_timeout $\geq$ silence\_timer.
...
\begin{itemize}
\item \textbf{Volume and Overhead (M1, M2):} We measure the Update Count and Total Output Volume in Character-Normalized Token Units (CTU) to quantify output volume and update frequency.
...
Full EXAIM (V0) consistently outperforms the turn-based baseline (V1) across core explainability metrics. Figure~\ref{fig1} provides a comprehensive comparison of all variants across Coverage (M4), Faithfulness (M6b), and Redundancy (M3) metrics.

\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{figure1/figure1.pdf}
\caption{Multi-Variant Comparison: Coverage, Faithfulness, and Redundancy metrics across all ablation variants. V0 (Full EXAIM) achieves the best balance across all metrics, with higher coverage and faithfulness than the turn-based baseline (V1) while maintaining lower redundancy.}
\label{fig1}
\end{figure}
...
Without semantic filtering, the system produces excessive, fragmented updates, increasing update frequency and output volume and lowering semantic yield per update. This confirms that syntax-aware segmentation alone is insufficient for real-time explainability.
...
Although V4 captures marginally more information, it does so at the cost of substantially higher interruption frequency. This demonstrates that novelty filtering functions as an update frequency control mechanism rather than a simple deduplication strategy.
...
The observed latency increase remains within acceptable bounds for interactive dashboard update latencies. While token usage is higher, this overhead reflects the cost of semantic analysis required for redundancy suppression and improved contract-groundedness and coverage. Figure~\ref{fig3} visualizes this trade-off, showing that the latency overhead is modest (<0.3s) despite the higher computational cost.

\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{figure3/figure3.pdf}
\caption{Computational Overhead Comparison: Average Summary Latency (s) and Relative Token Usage (×) for V0 (Full EXAIM) vs V1 (Turn-Based). The latency increase remains within acceptable bounds for interactive dashboard update latencies, while the higher token usage reflects the cost of semantic analysis required for redundancy suppression and improved contract-groundedness.}
\label{fig3}
\end{figure}
