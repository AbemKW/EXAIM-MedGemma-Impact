...
Through systematic DSR evaluation on 40 diagnostic replay cases, we verified that EXAIM represents an effective operating point under clinician-facing constraints. The observed magnitude of update suppression---approximately 75\% reduction relative to unfiltered streams---indicates that raw multi-agent LLM outputs contain a high ratio of semantic redundancy in clinical diagnostic contexts. This validates the necessity of semantic middleware over purely syntactic aggregation: without contextual novelty detection, the majority of generated content represents reformulation rather than information gain. The contrast between V0 and V1 further demonstrates that architectural turn boundaries, while natural structural triggers, fail to align with clinical information boundaries, yielding 26\% lower faithfulness. Critically, V4's 41\% increase in updates with negligible coverage benefit confirms that novelty filtering is not an optimization but a deployment prerequisite for managing alert fatigue in real-world CDSS.
