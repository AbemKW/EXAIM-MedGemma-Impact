...
Throughout the evaluation, "length constraints" are operationalized using a fixed reading-budget proxy introduced in Section~\ref{sec:experiments} to enable reproducible comparisons across variants.
...
We isolate the contribution of each architectural primitive through a controlled ablation study. Table~\ref{tab:ablation-variants} defines the five experimental configurations (V0--V4); the table is the sole listing of variant definitions to avoid redundancy with the body text.
...
\subsubsection{Operational Burden and Information Density}
...
$\Delta$ Coverage & +0.031 & +0.014 & +0.073 & +0.013 & +0.043 \\
\bottomrule
\end{tabular}
}
